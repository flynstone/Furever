...
namespace IdentityServerHost.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> LoginTwoStep(string email, bool rememberLogin, string returnUrl)
        {
            ...
            ViewData["RouteData"] = new Dictionary<string, string> { { "returnUrl", returnUrl }, { "email", email } };
            return View();
        }
        ...
        public async Task<IActionResult> LoginTwoStep(TwoStepModel twoStepModel, string returnUrl, string email)
        {
            ...
            {
                await HandleLockout(email, returnUrl); return View(twoStepModel);
            }
            else
            {
                return RedirectToAction(nameof(Error), new { returnUrl });
        ...
        {
            var user = await _userManager.FindByEmailAsync(email);
            var forgotPassLink = Url.Action(nameof(ForgotPassword), "Account", new { returnUrl }, Request.Scheme);
